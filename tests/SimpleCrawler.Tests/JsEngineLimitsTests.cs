...
public class JsEngineLimitsTests
{
    ...
    // Loose enough that a slow agent cannot fail these; the assertion is that the render returned at all.
    private static readonly TimeSpan _returnedAtAll = TimeSpan.FromSeconds(60);
    ...
    // rather than sharing the fixture's default-configured factories.
    private static ServiceProvider Provider(JsEngine engine, TimeSpan timeout)
    {
        var services = new ServiceCollection();
        if (engine == JsEngine.V8)
            services.AddV8JsEngine(new V8EngineOptions { PageTimeout = timeout });
        else
            services.AddJintJsEngine(new JintEngineOptions { ScriptTimeout = timeout });
    ...
    public async Task A_page_that_never_finishes_is_abandoned_when_its_ceiling_is_spent(JsEngine engine)
    {
        ...
        Assert.True(elapsed.Elapsed < _returnedAtAll, $"took {elapsed.Elapsed}");
    }
